...
void template_matching(unsigned char* bitmap, unsigned char* template, int bitmap_width, int bitmap_height, int template_width, int template_height, float mincorr)
{
	...
	for (i = 0; i < bitmap_height - template_height; i++)
		for (j = 0; j < bitmap_width - template_width; j++)
		{
			...
			if (corr > mincorr) draw_red(bitmap, bitmap_width, template_width, template_height, i, j);
			//printf("%f ", corr);
		}
	return;
}
unsigned char* draw_red(unsigned char* bitmap, int bitmap_width, int template_width, int template_height, int top_i, int top_j)
{
	...
	for (i = top_i; i < template_height + top_i; i++)
	{
		...
		{
			int offset = d2(i, j, bitmap_width);
			if (i == template_height + top_i - 1 || i == top_i)bitmap[3 * offset] = 255;
			if (j == template_width + top_j - 1 || j == top_j)bitmap[3 * offset] = 255;
